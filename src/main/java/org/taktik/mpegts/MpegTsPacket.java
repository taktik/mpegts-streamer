...
public class MpegTsPacket {
	...
	public MpegTsPacket(boolean transportErrorIndicator, boolean payloadUnitStartIndicator, boolean transportPriority, int pid, int scramblingControl, boolean adaptationFieldExist, boolean containsPayload, int continuityCounter, AdaptationField adaptationField, ByteBuffer payload) {
	...
	public static MpegTsPacket readPacket(ReadableByteChannel channel) throws IOException {
	...
	public static MpegTsPacket parsePacket(ByteBuffer buffer) {
		// Sync byte
		...
		MpegTsPacket.AdaptationField adaptationField = null;
		if (adaptationFieldExist) {
			...
			// PCR
			MpegTsPacket.AdaptationField.PCR pcr = null;
			if (pcrFlag) {
				...
				pcr = new MpegTsPacket.AdaptationField.PCR(base, extension);
			}

			// OPCR
			MpegTsPacket.AdaptationField.PCR opcr = null;
			if (opcrFlag) {
				...
				opcr = new MpegTsPacket.AdaptationField.PCR(base, extension);
			}
			...
			adaptationField = new MpegTsPacket.AdaptationField(discontinuityIndicator, randomAccessIndicator, elementaryStreamPriorityIndicator, pcrFlag, opcrFlag, splicingPointFlag, transportPrivateDataFlag, adaptationFieldExtensionFlag, pcr, opcr, spliceCountdown);
		}

		// Payload
		ByteBuffer payload = containsPayload ? buffer : null;

		return new MpegTsPacket(transportErrorIndicator, payloadUnitStartIndicator, transportPriority, pid, scramblingControl, adaptationFieldExist, containsPayload, continuityCounter, adaptationField, payload);
	}
}